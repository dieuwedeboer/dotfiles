...
16. Each widget is *in the bar*, not merely on disk: `omarchy-plugin-list --json | jq -r '.[] | select(.kinds | index("bar-widget")) | "\(.enabled)\t\(.id)"'` prints `true` for every row. For a widget `enabled` means its place in `shell.json` `bar.layout`, so a plugin directory that exists with no bar row reads as installed and shows nothing. That is the failure this line exists to catch.
17. `/usr/share/omarchy` is a symlink to `/usr/local/share/omarchy`, and `flea` launches with its file listing themed. Both `/usr/share/flea/ui/Commons` and `/usr/share/flea/ui/Ui` resolve through that link.
