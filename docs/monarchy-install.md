...
- Install `monarchy/plugins` into `~/.config/omarchy/plugins/<id>/`. Current list is Grok usage (`omarchy plugin add https://github.com/dougfour/omarchy-grok-usage.git --enable`) and Activity Monitor (`omarchy plugin add https://github.com/stappmus/omarchy-activity-monitor.git --enable`). `--enable` writes `shell.json` `plugins[]`. Does not need a live Omarchy session. Add more rows to that list to grow the collection. `omarchy plugin update` is still the updater for a checkout that already exists.
