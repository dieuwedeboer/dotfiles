...
`open-fprintd` Provides the fprintd D-Bus API and **conflicts** with the `fprintd` package. Omarchy remove/setup will fight it on every run that touches fingerprint packages. That fight is why this stays out of `hardware/hp-zbook/apply.sh` and out of Monarchy apply.
