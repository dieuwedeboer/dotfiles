...
fail() {
    echo "test-sddm-resume: $*" >&2
    exit 1
}

cmd="$SCRIPT_DIR/sddm-resume.sh"
...
grep -q 'monarchy_install_sddm_resume' "$SCRIPT_DIR/sddm.sh" \
    || fail "sddm.sh does not install the resume helper"
echo "$(awk '/^monarchy_keep_sddm\(\)/,/^}$/' "$SCRIPT_DIR/sddm.sh")" \
    | grep -q 'monarchy_install_sddm_resume' \
    || fail "monarchy_keep_sddm does not install the resume helper"
grep -q 'sddm-resume.sh' "$SCRIPT_DIR/update.sh" \
    || fail "check/apply does not require sddm-resume.sh"
grep -q 'monarchy-sddm-resume.service' "$SCRIPT_DIR/sddm.sh" \
